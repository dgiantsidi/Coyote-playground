...
constexpr auto const defNRepsLat = 15000000;
...
int main(int argc, char *argv[]) {
	...
	uint64_t *hMem =
		reinterpret_cast<uint64_t *>(iqp->getQpairStruct()->local.vaddr);
	iqp->ibvSync(mstr);
	...
	if (mstr) {
		...
		sg.type.rdma.len = sizeof(int) + size;
#endif
		...
		auto benchmark_lat = [&]() {
			...
			hMem[max_size / 2 - 1] =
				1; /* here add a faulty byte to check if the following RDMA_READ will
				      overwrite properly */
			for (auto idx = 0; idx < max_size; idx++) {

				// KAMIL
				std::cout << std::hex << hMem[idx] << " ";
				if ((idx + 1) % size == 0)
					std::cout << "\n";
			}
			fmt::print("\n");
		...
		if (oper) {
			...
			for (auto idx = 0; idx < max_size; idx++) {
				...
				// KAMIL
				std::cout << std::hex << static_cast<int>(hMem[idx]) << " ";
				if (((idx + 1) % size) == 0)
					fmt::print("\n");
			}
