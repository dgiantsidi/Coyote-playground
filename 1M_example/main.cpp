...
constexpr auto const defNRepsLat = 1000000;
constexpr auto const defMinSize = 64;
constexpr auto const defMaxSize = 64 * 4;
...
int main(int argc, char *argv[]) {
	...
	uint64_t *hMem = reinterpret_cast<uint64_t*>(iqp->getQpairStruct()->local.vaddr);
	iqp->ibvSync(mstr);
	...
	if (mstr) {
		...
		// sg.type.rdma.len = sizeof(uint64_t)*size;
		sg.type.rdma.len = sizeof(uint64_t)*2;
#endif
#if 0
		sg.type.rdma.local_offs = min_size;
		sg.type.rdma.remote_offs = min_size;
		...
		auto benchmark_lat = [&]() {
...
#if 1
				sg1.type.rdma.local_offs = min_size;
				sg1.type.rdma.remote_offs = min_size;
				// sg1.type.rdma.len = max_size * sizeof(int) /2 * 2;
				sg1.type.rdma.len = sizeof(uint64_t)*2;
#endif
#if 0
			...
			sg1.type.rdma.len = sizeof(uint64_t);
#endif
		...
		if (oper) {
			...
			while (true) {
				...
				auto done_ops = iqp->ibvDone();
				if (iqp->ibvDone() % 10 == 0 || done_ops % 10 == 1 || done_ops % 10 == 2) {
				for (auto idx = 0; idx < max_size; idx++) {
					fmt::print("{}", hMem[idx]);
					if ((idx + 1) % size == 0)
						std::cout << "\n";
