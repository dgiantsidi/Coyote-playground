...
#pragma HLS array_partition variable = b0.M complete
    ...
    ap_uint<512> inp512 = msg_strm.read();
//        std::cout<<std::hex<<inp512<<std::endl;
    ap_uint<5> index;
    LOOP_GEN_ONE_FULL_BLK:
        for (ap_uint<5> i = 0; i < 16; ++i) {
#pragma HLS unroll
            ...
            if(index == 8) {
                index =0;
                inp512 = msg_strm.read();
//                    std::cout<<std::hex<<inp512<<std::endl;
            } else if(index >8) {
                index = index -8;
            }
            ap_uint<64> l = inp512((7-index+1)*64-1, (7-index)*64);
            // XXX algorithm assumes big-endian
#ifdef BE
            l = ((0x00000000000000ffUL & l) << 56) | ((0x000000000000ff00UL & l) << 40) |
                ((0x0000000000ff0000UL & l) << 24) | ((0x00000000ff000000UL & l) << 8) |
...
#pragma HLS unroll
        ...
        ap_uint<8> byte6 = ((l >> 8) & 0xff);
        ap_uint<8> byte7 = (l & 0xff);
        digest.range(64 * i + 64 - 1, 64 * i) = ((ap_uint<64>)byte0) | (((ap_uint<64>)byte1) << 8) |
                                                (((ap_uint<64>)byte2) << 16) | (((ap_uint<64>)byte3) << 24) |
                                                (((ap_uint<64>)byte4) << 32) | (((ap_uint<64>)byte5) << 40) |
                                                (((ap_uint<64>)byte6) << 48) | (((ap_uint<64>)byte7) << 56);
    }
    // obtain the digest by trancating the left-most hash_width bits of 512-bit hash value
    digest_strm.write(digest.range(hash_width - 1, 0));
...
#pragma HLS resource variable = w_strm core = FIFO_LUTRAM
    ...
    // padding and appending message words into blocks
    preProcessing(msg_strm, len, blk_strm, nblk_strm);

    // duplicate number of block stream and its end flag stream
    dup_strm<128>(nblk_strm, nblk_strm1, nblk_strm2);

    // generate the message schedule in stream
    generateMsgSchedule(blk_strm, nblk_strm1, w_strm);
//
//    // digest precessing blocks into hash value
    SHA512Digest<64, hash_width>(w_strm, nblk_strm2, digest_strm);
...
void sha512(
    ...
    // outputs
    hls::stream<ap_uint<512> >& digest_strm) {
    internal::sha512Top<w, 512>(msg_strm, len, // input streams
                                digest_strm);    // output streams
