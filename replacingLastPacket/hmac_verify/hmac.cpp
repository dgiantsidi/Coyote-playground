...
void injectData(hls::stream<AXIS_DATA>& input, hls::stream<ap_uint<512> >& data_strm,
                hls::stream<ap_uint<40> >& len_strm,
                ap_uint<384> key,
                ap_uint<512>& keep,
                ap_uint<6>& id,
                ap_uint<512>& last_input
                ) {
/////
    ap_uint<5> tmp_len;
    ap_uint<64> tmp_data;
...
#pragma HLS resource variable = tid_strm core = FIFO_LUTRAM
    ...
    preProcessing(input, blk_strm, end_nblk_strm, tkeep_strm, tid_strm, last_input);
...
ap_uint<64> my_switchEndian(ap_uint<64> l) {
    return ((0x00000000000000ffUL & l) << 56) | ((0x000000000000ff00UL & l) << 40) | ((0x0000000000ff0000UL & l) << 24) |
        ((0x00000000ff000000UL & l) << 8) | ((0x000000ff00000000UL & l) >> 8) | ((0x0000ff0000000000UL & l) >> 24) |
        ((0x00ff000000000000UL & l) >> 40) | ((0xff00000000000000UL & l) >> 56);
}
...
static void preProcessingOneTripForHMAC(ap_uint<2> op_type,
                                        ...
                                        hls::stream<ap_uint<40> >& len_strm,
                                        hls::stream<ap_uint<65*8> >& blk_strm) {
    ap_uint<128> total_len = 192;
    ap_uint<4> counter = 0;
    ap_uint<5> tmp_len;
    ap_uint<40> HUH;
    ap_uint<512> bigHUH;
    ap_uint<65*8> bigBLOCK;
    if (op_type == 1) { // for "ipad + msg" hash
        ...
        if (HUH.range(34,30) == 16 && HUH.range(29,25) ==24) { // TODO
            ...
            opad_buffer[0] = ll0 ^ ap_uint<64>("0x5c5c5c5c5c5c5c5c");
            opad_buffer[1] = ll1 ^ ap_uint<64>("0x5c5c5c5c5c5c5c5c");
            opad_buffer[2] = ll2 ^ ap_uint<64>("0x5c5c5c5c5c5c5c5c");
            opad_buffer[3] = ll3 ^ ap_uint<64>("0x5c5c5c5c5c5c5c5c");
            opad_buffer[4] = ll4 ^ ap_uint<64>("0x5c5c5c5c5c5c5c5c");
            opad_buffer[5] = ll5 ^ ap_uint<64>("0x5c5c5c5c5c5c5c5c");
            ...
            ipad = my_switchEndian(ap_uint<64>("0x3636363636363636"));

            bigBLOCK.range(64,0) = my_switchEndian(ll0 ^ ap_uint<64>("0x3636363636363636"));
            bigBLOCK.range(129,65) = my_switchEndian(ll1 ^ ap_uint<64>("0x3636363636363636"));
            bigBLOCK.range(194,130) = my_switchEndian(ll2 ^ ap_uint<64>("0x3636363636363636"));
            bigBLOCK.range(259,195) = my_switchEndian(ll3 ^ ap_uint<64>("0x3636363636363636"));

            bigBLOCK.range(324, 260) = my_switchEndian(ll4 ^ ap_uint<64>("0x3636363636363636"));
            bigBLOCK.range(389, 325) = my_switchEndian(ll5 ^ ap_uint<64>("0x3636363636363636"));
            bigBLOCK.range(454, 390) = ipad;
            bigBLOCK.range(519, 455) = ipad;
            blk_strm.write(bigBLOCK);

            bigBLOCK.range(64, 0)    = ipad;
            bigBLOCK.range(129, 65)  = ipad;
            bigBLOCK.range(194, 130) = ipad;
            bigBLOCK.range(259, 195) = ipad;

            bigBLOCK.range(324, 260) = ipad;
            bigBLOCK.range(389, 325) = ipad;
            bigBLOCK.range(454, 390) = ipad;
            bigBLOCK.range(519, 455) = ipad;
            blk_strm.write(bigBLOCK);


            bigBLOCK.range(64, 0)    ="9223372036854775808";
            bigBLOCK.range(129, 65)  = 0;
            bigBLOCK.range(194, 130) =0;
            bigBLOCK.range(259, 195) =0;

            bigBLOCK.range(324, 260) =0;
            bigBLOCK.range(389, 325) =0;
            bigBLOCK.range(454, 390) =0;
            bigBLOCK.range(519, 455) =0;
            blk_strm.write(bigBLOCK);

            bigBLOCK.range(64, 0)    =0;
            bigBLOCK.range(129, 65)  =0;
            bigBLOCK.range(194, 130) =0;
            bigBLOCK.range(259, 195) =0;
            ...
            ll6 = my_switchEndian(bigHUH.range(447, 384));
            ll7 = my_switchEndian(bigHUH.range(511, 448));
            ipad = my_switchEndian(ap_uint<64>("0x3636363636363636"));
            ...
            bigBLOCK.range(64, 0)    = ll6;
            bigBLOCK.range(129, 65)  =ll7;

            bigHUH = msg_strm.read();

            bigBLOCK.range(194, 130) =my_switchEndian(bigHUH.range(63, 0));
            bigBLOCK.range(259, 195) =my_switchEndian(bigHUH.range(127, 64));

            bigBLOCK.range(324, 260) =my_switchEndian(bigHUH.range(191, 128));
            bigBLOCK.range(389, 325) =my_switchEndian(bigHUH.range(255, 192));
            bigBLOCK.range(454, 390) =my_switchEndian(bigHUH.range(319, 256));
    ...
    #pragma HLS pipeline II = 1
                total_len += 56;
                counter+=8;
                bigBLOCK.range(519, 455) =my_switchEndian(bigHUH.range(383, 320));
                blk_strm.write(bigBLOCK);


                bigBLOCK.range(64, 0)    =my_switchEndian(bigHUH.range(447, 384));
                bigBLOCK.range(129, 65)  =my_switchEndian(bigHUH.range(511, 448));
                bigHUH = msg_strm.read();
                bigBLOCK.range(194, 130) =my_switchEndian(bigHUH.range(63, 0));
                bigBLOCK.range(259, 195) =my_switchEndian(bigHUH.range(127, 64));
        ...
        bigBLOCK.range(64, 0)    = my_switchEndian(opad_buffer[0]);
        bigBLOCK.range(129, 65)  = my_switchEndian(opad_buffer[1]);
        bigBLOCK.range(194, 130) = my_switchEndian(opad_buffer[2]);
        bigBLOCK.range(259, 195) = my_switchEndian(opad_buffer[3]);

        bigBLOCK.range(324, 260) = my_switchEndian(opad_buffer[4]);
        bigBLOCK.range(389, 325) = my_switchEndian(opad_buffer[5]);
        bigBLOCK.range(454, 390) = my_switchEndian(opad_buffer[6]);
        bigBLOCK.range(519, 455) = my_switchEndian(opad_buffer[7]);

        blk_strm.write(bigBLOCK);

        bigBLOCK.range(64, 0)    = my_switchEndian(opad_buffer[8]);
        bigBLOCK.range(129, 65)  = my_switchEndian(opad_buffer[9]);
        bigBLOCK.range(194, 130) = my_switchEndian(opad_buffer[10]);
        bigBLOCK.range(259, 195) = my_switchEndian(opad_buffer[11]);

        bigBLOCK.range(324, 260) = my_switchEndian(opad_buffer[12]);
        bigBLOCK.range(389, 325) = my_switchEndian(opad_buffer[13]);
        bigBLOCK.range(454, 390) = my_switchEndian(opad_buffer[14]);
        bigBLOCK.range(519, 455) = my_switchEndian(opad_buffer[15]);

        blk_strm.write(bigBLOCK);

        bigBLOCK.range(64, 0)    =my_switchEndian(digest_input.range(63, 0));
        bigBLOCK.range(129, 65)  =my_switchEndian(digest_input.range(127, 64));
        bigBLOCK.range(194, 130) =my_switchEndian(digest_input.range(191, 128));
        bigBLOCK.range(259, 195) =my_switchEndian(digest_input.range(255, 192));

        bigBLOCK.range(324, 260) =my_switchEndian(digest_input.range(319, 256));
        bigBLOCK.range(389, 325) =my_switchEndian(digest_input.range(383, 320));
        bigBLOCK.range(454, 390) ="9223372036854775808";
        bigBLOCK.range(519, 455) =0;
        ...
        bigBLOCK.range(389, 325) = 0;
        bigBLOCK.range(454, 390) = 0;
        bigBLOCK.range(519, 455) ="18446744073709553024";
...
void generateMsgScheduleOneTrip(hls::stream<ap_uint<65*8> >& blk_strm, ap_uint<hash_width>& res) {
    static const ap_uint<64> K[80] = {
        0x428a2f98d728ae22UL, 0x7137449123ef65cdUL, 0xb5c0fbcfec4d3b2fUL, 0xe9b5dba58189dbbcUL, 0x3956c25bf348b538UL,
        0x59f111f1b605d019UL, 0x923f82a4af194f9bUL, 0xab1c5ed5da6d8118UL, 0xd807aa98a3030242UL, 0x12835b0145706fbeUL,
        0x243185be4ee4b28cUL, 0x550c7dc3d5ffb4e2UL, 0x72be5d74f27b896fUL, 0x80deb1fe3b1696b1UL, 0x9bdc06a725c71235UL,
...
#pragma HLS array_partition variable = H complete
    ...
    #pragma HLS array_partition variable = W
        ...
        ap_uint<64> kamilITER = 0;
        ap_uint<65*8> bigSTRM = blk_strm.read();
        while (run || counter != 0) {
    #pragma HLS UNROLL
            if (counter == 0) {
                a = H[0];
            ...
            if (counter < 16) {
                ...
                if (kamilITER < 8){
                    tmp_Wt = bigSTRM.range(64+65*kamilITER,65*kamilITER);
                    kamilITER++;
                } else {
                    bigSTRM = blk_strm.read();
                    tmp_Wt = bigSTRM.range(64, 0);
                    kamilITER = 1;
                }
...
#pragma HLS dataflow
    hls::stream<ap_uint<65*8> > blk_strm("blk_strm");
#pragma HLS stream variable = blk_strm depth = 32
#pragma HLS resource variable = blk_strm core = FIFO_LUTRAM
    preProcessingOneTripForHMAC(op_type, opad_buffer, digest_input, msg_strm, len_strm, blk_strm);
 ...
 */
void HMAC_SHA384(hls::stream<AXIS_DATA>& input, hls::stream<AXIS_DATA>& output, ap_uint<512>& last_input) {
/**
 * isHMAC: false -> SHA384.
 ...
 */
#pragma HLS dataflow
#pragma HLS allocation function instances = internal::HMAC_SHA384_2in1 limit = 2
    ap_uint<64> opad_buffer[16];
...
#pragma HLS resource variable = len_strm core = FIFO_LUTRAM
    ...
    ap_uint<384> key ="0x303132333435363738393031323334353637383930313233343536373839303132333435363738393031323334353637";
    ap_uint<384> res;
    injectData(input, msg_key_strm, len_strm, key, keep, id, last_input);
...
void hmac(hls::stream<AXIS_DATA>& input,
		hls::stream<AXIS_DATA >& output) {
...
#pragma HLS AGGREGATE variable=output bit
	ap_uint<512> last;
	AXI::xf::security::HMAC_SHA384(input, output, last);
}

void hmac_verify(hls::stream<AXIS_DATA>& input,
...
#pragma HLS resource variable = output2 core = FIFO_LUTRAM
	...
	AXI::xf::security::HMAC_SHA384(input, output2, last);
	ap_uint<1> result = 0;

	AXIS_DATA outWord;
	output2.read(outWord);
	if ( outWord.tdata != last ) {
		result = 1;
	}
	output.write(result);
