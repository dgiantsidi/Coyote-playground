...
__attribute__((sdx_kernel("sha512_verify", 0))) void sha512_verify(hls::stream<AXIS_DATA>& input,
...
#pragma HLS array_partition variable = b0.M complete
 ...
 ap_uint<512> inp512c = inputBuffer[0];
            ...
            ap_uint<5> index;
                VITIS_LOOP_114_1: for (ap_uint<5> i = 0; i < 16; ++i) {
#pragma HLS unroll
 index = i;
                    if(index == 8) {
                        index =0;
...
#pragma HLS array_partition variable = b0.M complete
 ...
 ap_uint<512> inp512c = inputBuffer[0];
            ...
            ap_uint<5> index;
                VITIS_LOOP_191_2: for (ap_uint<5> i = 0; i < 16; ++i) {
#pragma HLS unroll
 index = i;
                    if(index == 8) {
                        index =0;
...
#pragma HLS pipeline II = 1
 ap_uint<64> Wt = SSIG1<64>(W[14]) + W[9] + SSIG0<64>(W[1]) + W[0];
                    VITIS_LOOP_521_1: for (ap_uint<5> i = 0; i < 15; i++) {
                        W[i] = W[i + 1];
                    }
                    W[15] = Wt;
                    w_strm.write(Wt);
...
static void preProcessingOneTripForHMAC(ap_uint<2> op_type,
    ...
    if (op_type == 1) {
        ...
        VITIS_LOOP_830_1: for (int i = 0; i < 16; i++) {
#pragma HLS pipeline II = 1
 ap_uint<64> l;
            if (key_not_finished) {
                l = msg_strm.read();
        ...
        VITIS_LOOP_848_2: for (int i = 0; i < 16; i++) {
#pragma HLS pipeline II = 1
 ap_uint<64> l = opad_buffer[i];
            switchEndian(l);
            blk_strm.write(l);
        }
        VITIS_LOOP_854_3: for (int i = 0; i < hash_width / 64; i++) {
#pragma HLS pipeline II = 1
 ap_uint<64> l = digest_input.range(i * 64 + 63, i * 64);
    ...
    if (op_type == 0 || op_type == 1) {
        ...
        VITIS_LOOP_873_4: while (tmp_len[4] != 1) {
#pragma HLS pipeline II = 1
 counter++;
...

    {
        ...
        VITIS_LOOP_917_5: for (ap_uint<5> z = 0; z < z_rounds; z++) {
            blk_strm.write(0);
        }
    }
...
#pragma HLS array_partition variable = W
 ap_uint<64> a, b, c, d, e, f, g, h;
    VITIS_LOOP_999_1: while (run || counter != 0) {
#pragma HLS pipeline II = 1
 if (counter == 0) {
            a = H[0];
            b = H[1];
        ...
        if (counter < 16) {
            ...
            VITIS_LOOP_1022_2: for (ap_uint<5> i = 0; i < 15; i++) {
                W[i] = W[i + 1];
            }
            W[15] = Wt;
        }
...
# 1291 "SHA512CODE/sha512.cpp"
void HMAC_SHA384(bool isHMAC,
                 hls::stream<ap_uint<32> >& msg_key_strm,
                 hls::stream<ap_uint<4> >& len_strm,
                 ap_uint<384>& res) {
# 1309 "SHA512CODE/sha512.cpp"
#pragma HLS allocation function instances = internal::HMAC_SHA384_2in1 limit = 1
 ap_uint<64> opad_buffer[16];
    ap_uint<384> first_digest;
...
__attribute__((sdx_kernel("sha512_verify", 0))) void sha512_verify(hls::stream<AXIS_DATA>& input,
  hls::stream<ap_uint<1> >& output) {
#line 15 "/home/kamil/Desktop/DESHA512_01/solution1/csynth.tcl"
#pragma HLSDIRECTIVE TOP name=sha512_verify
# 1344 "SHA512CODE/sha512.cpp"
